...
<div  ng-app="myApp" ng-controller="customersCtrl">
	...
	<div ng-show="P3" class="container-display text-center" style="width:550px;">
...
<script>
	...
	app.controller('customersCtrl', function($scope, $http) {
		...
		$scope.P1send = function() {
		    $http({
			    method: "post",
			    url: "dispTest.inc.php?page=bill1",
			    data: {
			        'cid' : $scope.P1cid,
			    }
			}).then(function (data) {
		...
		$scope.P2add = function() {
			...
			if($scope.pname != '') {
				$http({
				    method: "post",
				    url: "dispTest.inc.php?page=bill2",
				    data: {
				        'name' : $scope.pname,
				        'rpppu': $scope.rpppu,
				        'pppu' : $scope.pppu,
				   	...
				   	if (data.data.status == 'success')
				   	{
						...
						if($scope.P2disp1 == false && $scope.pnum != '' && $scope.pnum >0 && $scope.pnum <= $scope.panum
							...
							$scope.prod.push(
							{
								...
								"ppu":$scope.pppu,
								"num":$scope.pnum,
								"price":($scope.pnum * $scope.pppu)
							});
							$scope.P2total += $scope.prod[$scope.P2num].price;
							$scope.P2num++;
		...
		$scope.bill = function() {
			if($scope.prod != '') {
				$http({
				    method: "post",
				    url: "dispTest.inc.php?page=bill3",
				    data: {
				    	'cid'  : $scope.P2cid,
				    	'prod': $scope.prod
				    }
